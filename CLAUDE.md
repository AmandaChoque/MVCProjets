...
4. **Contratos**: Separados en `ContratoEmpleado` y `ContratoProyecto` (en `empleados/`). Los empleados tienen jornadas diarias registradas.
...
6. **Pagos**: Pagos del cliente al proyecto (`Pago` en `projects/`) y de la empresa al empleado (`PagoEmpleado` en `empleados/`). Señal Django actualiza `estado_pago` del proyecto.
...
URL routing: `project_management/urls.py` is the root; it `include()`s `empleados.urls` and `inventario.urls`. Payment and project URLs are defined directly in the root `urls.py`.

### `projects/` — Core app
- `models.py` — core models: AuditModel, Cliente, Proyecto, HistorialPresupuesto, Progreso, Sede, FotoSede, TareaChecklist, Notificacion, Pago
- `views.py` — project/cliente/sede/notificacion/pago views (~1730 lines, function-based)
...
- `signals.py` — project-level signals
- `validators.py` — password validators (UppercaseValidator, LowercaseValidator, NumberValidator)
- `templates/` — all HTML templates (~50+ files), extend `base.html` in `templates/`

### `empleados/` — Employee app
- `models.py` — Empleado (AbstractUser), ContratoEmpleado, ContratoProyecto, PagoEmpleado, JornadaEmpleado
- `views.py`, `urls.py`, `forms.py`
- Employee, contract, jornada, and employee-payment views all live here
...
|---|---|---|
| `AuditModel` | created, updated_at, deleted_at, deleted_by, activo | Abstract base for all models (imported by other apps) |
...
| `Proyecto` | codigo, nombre, descripcion, estado_proyecto, tipo_proyecto, fecha_inicio, fecha_fin, observacion, estado_pago, monto_total, FK→Cliente/creado_por, M2M→Empleado(equipo) | `_sync_estado_pago()` called by post_save/post_delete on Pago. `equipo` M2M via `empleados.Empleado` |
...
| `Sede` | FK→Proyecto, nombre, direccion, descripcion, latitud, longitud, estado | Installation point. `porcentaje_checklist` and `_sync_estado()` based on TareaChecklist |
...
| `Notificacion` | FK→destinatario(User), tipo, mensaje, leida, FK→Proyecto, FK→Sede, fecha | Types: sede_completada/tarea_completada/general |
| `Pago` | FK→Proyecto, monto, fecha, tipo_pago, numero_referencia | Post-save/delete → `proyecto._sync_estado_pago()` |

### `empleados/models.py`

| Model | Key fields | Notes |
|---|---|---|
| `Empleado` | nombre, apellido_paterno, apellido_materno, cargo, carnet_identidad (unique), numero_celular | **Extends `AbstractUser` directly**. cargo choices: administrador/gerente/instalador/tecnico_soporte/secretaria |
| `ContratoEmpleado` | FK→Empleado, tipo_salario, fecha_firma, fecha_inicio, fecha_fin, monto_acordado, observaciones, documento | `monto_diario` property. UniqueConstraint: one active per empleado |
| `ContratoProyecto` | FK→Proyecto, fecha_firma, fecha_inicio, fecha_fin, monto_acordado | Contract between empresa and client per project |
| `PagoEmpleado` | FK→ContratoEmpleado, monto, fecha, tipo_pago, concepto | concepto: pago_jornada/adelanto/liquidacion/dia_extra/otro. UniqueConstraint: one active liquidacion per contrato |
| `JornadaEmpleado` | FK→ContratoEmpleado, FK→Proyecto, fecha, dias (0.5 or 1.0), observacion, FK→PagoEmpleado (nullable) | `monto` property = dias × contrato.monto_diario |
...
| `Requiere` | FK→Proyecto, FK→Insumo, cantidad, costo_unitario | `unique_together (proyecto, insumo)`. `subtotal` property |
| `Compra` | FK→Proveedor, FK→Insumo, cantidad, costo_unitario, costo_total, fecha | Triggers stock recalculation via signal |
...
**Forms**: All forms use Bootstrap `form-control`/`form-select` widgets. Decimal fields (monto, costo_unitario) use `CharField` + `clean_*` with regex `r'\d+(\.\d{1,2})?'` — no commas, dot as decimal separator. Forms live in `form.py` (projects) or `forms.py` (empleados, inventario).
...
**Role-based access**: `@cargo_required(*cargos)` from `projects/decorators.py` after `@login_required`. Predefined groups: `ROLES_ADMIN = ('administrador', 'gerente')`, `ROLES_ADMIN_SEC` (adds secretaria), `ROLES_CAMPO` (adds instalador, tecnico_soporte). Superusers bypass all checks. Templates use `{% if es_admin %}` / `{% if es_campo %}` etc.

**Empleado is the User model**: `AUTH_USER_MODEL = 'empleados.Empleado'`. Access the current user's cargo via `request.user.cargo`.
...
# Pagos del cliente al proyecto (projects/views.py, defined in root urls.py)
...
/employees/, /employees/create/, /employees/carga/
/employees/<id>/, /employees/<id>/view/, /employees/<id>/deactivate/
/mi-trabajo/                                 → instalador dashboard

# Contratos de empleado (empleados/urls.py)
/employees/<id>/contratos/nuevo/
