...
from .validators import DECIMAL_REGEX, parse_decimal, validate_phone, validate_numeric_id, validate_document_ext
...
class ClienteForm(forms.ModelForm):
    ...
    def clean_nit_ci(self):
        return validate_numeric_id(self.cleaned_data.get('nit_ci', ''), min_len=6)
    ...
    def clean_telefono(self):
        return validate_phone(self.cleaned_data.get('telefono', ''), min_len=7, required=False)
...
class PaymentForm(forms.ModelForm):
    ...
    def clean_monto(self):
        resultado = parse_decimal(self.cleaned_data.get('monto', ''))
        if resultado <= 0:
            raise forms.ValidationError('El monto debe ser mayor a cero.')
        return resultado
